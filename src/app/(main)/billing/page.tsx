            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className='whitespace-nowrap'>Payable</TableHead>
              ...
              <TableBody>
                {billingData?.installments.map((installment, index) => (
                  <TableRow key={index}>
                    <TableCell>{installment.number.split(' ').at(0)}</TableCell>
                  ...
                  <TableRow key={transaction.slNo}>
                    ...
                    <TableCell
                      className={getAmountColor(
                        transaction.amount || -transaction.payment || transaction.discount || 0
                      )}
                    >
                      ৳{transaction.amount || -transaction.payment || transaction.discount || 0}
                    </TableCell>
                    ...
                    <TableCell className='flex items-center justify-center lg:justify-start lg:xitems-start'>
                      <div className='flex flex-col gap-2 items-center justify-center lg:flex-row lg:flex-wrap lg:justify-start lg:items-start '>
                        {transaction.courseCode && (
                          <Badge variant='outline' className='mr-1 text-center bg-purple-100'>
                            {transaction.courseCode}
                          </Badge>
                        )}
                        {transaction.trimesterName && (
                          <Badge variant='outline' className='mr-1 text-center bg-orange-100'>
                            {transaction.trimesterName}
                          </Badge>
                        )}
                        {transaction.credit && (
                          <Badge variant='outline' className='mr-1 text-center bg-blue-100'>
                            {transaction.credit} credits
                          </Badge>
                        )}
                        {transaction.discount && (
                          <Badge variant='outline' className='mr-1 text-center bg-green-100'>
                            {transaction.credit} Discounts
                          </Badge>
                        )}
                        {transaction.payment && (
                          <Badge variant='outline' className='mr-1 text-center bg-green-100'>
                            {transaction.credit} Payments
                          </Badge>
                        )}
                      </div>
                    </TableCell>
